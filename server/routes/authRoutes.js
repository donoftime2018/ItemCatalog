...
app.post("/resetPasswordLink", async (req, res) => {
    ...
    try
    {
        ...
        else
        {
            ...
            sendMail(email, `<a href="${process.env.HOSTED_CLIENT}/updatePassword/${token}">Click here to reset your password.</a> <p>You only have 10 minutes to reset it.</p>`, "Reset Password")
            res.status(200).send()
        }
    } catch(err) {
        res.status(400).send({msg: err})
